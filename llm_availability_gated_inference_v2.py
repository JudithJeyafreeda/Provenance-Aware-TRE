...
DEFINITIONS = """You are a clinical temporal-relation extraction system. The note describes a
diagnosis event (EVENT1) and a treatment event (EVENT2).
...
- EVENT1-OVERLAP-EVENT2: the diagnosis and treatment occurred concurrently / during the same workup.

"""

LABELS = {
    "none": "llm_provenance_first_availability_gated",
    "convention": "llm_availability_gated_event_defs",
    "columns": "llm_availability_gated_event_cols",
}


def system_prompt(mode: str) -> str:
    return SCHEMA if mode == "none" else DEFINITIONS + SCHEMA


def user_message(row, mode: str, probe=("", "")) -> str:
    prefix, suffix = probe
    text = str(row["text"])
    if mode == "columns":
        body = f"EVENT1: {row['event1']}\nEVENT2: {row['event2']}\n\nNote:\n{text}"
    else:
        body = text
...
def call(client, model, system, user, temperature):
...
# (prefix, suffix) pairs; identical to the v1 re-query wording
PROBES = [
...
def necessity(client, model, system, row, mode, relation, probes, temperature):
    ...
    for probe in PROBES[:probes]:
        out = call(client, model, system, user_message(row, mode, probe), temperature)
        matches += int(out.get("relation") == relation)
    return matches / max(1, min(probes, len(PROBES)))


def one(row, client, args, model_name):
    text = str(row["text"])
    system = system_prompt(args.event_mode)
    try:
        out = call(client, args.model, system, user_message(row, args.event_mode), args.temperature)
...
def run(df, client, args, model_name):
    ...
    if args.workers <= 1:
        results = [one(r, client, args, model_name) for r in rows]
    else:
        with ThreadPoolExecutor(max_workers=args.workers) as ex:
            results = list(ex.map(lambda r: one(r, client, args, model_name), rows))
...
def main():
    ...
    p.add_argument("--event_mode", choices=["none", "convention", "columns"], default="convention")
    p.add_argument("--model_label", default=None, help="override the model label written to the output")
    p.add_argument("--workers", type=int, default=1, help="parallel requests (vLLM handles 8-16 fine)")
    ...
    label = args.model_label or LABELS[args.event_mode]
    result = run(df, client, args, label)
    ...
    with open(os.path.join(args.out_dir, f"llm_availability_gated_{stem}_metadata.json"), "w") as f:
        json.dump({"config": vars(args), "n": len(df), "model_label": label}, f, indent=2)
